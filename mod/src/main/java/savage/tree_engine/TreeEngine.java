...
public class TreeEngine implements ModInitializer {
    ...
    public void onInitialize() {
        ...
        net.fabricmc.fabric.api.event.lifecycle.v1.ServerLifecycleEvents.SERVER_STARTING.register(server -> {
            ...
            // Initialize tree generation executor
            savage.tree_engine.web.TreeGenerationExecutor.initialize();
        });

        net.fabricmc.fabric.api.event.lifecycle.v1.ServerLifecycleEvents.SERVER_STARTED.register(server -> {
            // Optionally auto-start the web editor once the server has fully finished loading
            // (used by standalone launcher setups so no manual command is required)
            if (savage.tree_engine.config.MainConfig.get().auto_start_web_on_boot) {
                savage.tree_engine.web.WebEditorServer.start(server);
            }
        });

        net.fabricmc.fabric.api.event.lifecycle.v1.ServerLifecycleEvents.SERVER_STOPPING.register(server -> {
            // Shutdown tree generation executor
            savage.tree_engine.web.TreeGenerationExecutor.shutdown();
